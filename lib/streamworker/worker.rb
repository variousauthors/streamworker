module Streamworker
  module Workers
    class Worker
      ...
      attr_accessor :repeats, :debug_throttling # for debugging throttling, will repeat 
    ...
    <html class="white"#{scroll}>
      ...
      def report_line(str, opts={})
        ...
        dbg = debug_throttling ? %Q{<span class="text-error">#{ShopifyAPI.credit_left}</span> } : ""
        start = fragment? ?  "" : %Q{
            <p class="#{p_class}">#{dbg}}
        @fragment = ! opts[:close]
        close = ""
